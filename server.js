...
app.get('/orcamento/:id/pdf', (req, res) => {
    ...
    doc.fontSize(16).text('SOLICITAÇÃO DE ORÇAMENTO', { align: 'center' }).moveDown();
    ...
    doc.text(`Produto: ${item.tipo_produto.toUpperCase()} | Qtd: ${item.quantidade}`);
    if (item.tipo_produto === 'disco') {
        doc.text(`Ângulo de corte: ${item.angulo_corte}`);
        doc.text(`Perfil: ${item.perfil_corte_disco}`);
    }
