...
app.post('/orcamento', upload.single('foto'), (req, res) => {
    try {
        ...
        const novo = {
            ...
            // Dados Técnicos (Os que você pediu)
...
app.get('/orcamento/:id/pdf', (req, res) => {
    ...
    doc.text(`Data: ${item.data}`).moveDown();
    ...
    doc.text(`Tipo de Fio: ${item.tipo_fio || 'N/A'}`);
    ...
    if (item.foto) {
        ...
        if (fs.existsSync(imgPath)) {
            doc.addPage().text('IMAGEM ANEXA:').image(imgPath, { fit: [450, 450] });
        }
    }
    doc.end();
});
